...
DF_SUPERDATASET = pd.read_pickle('casual_annotated.pkl')
DF_SUPERDATASET = DF_SUPERDATASET[['face embedding', 'fitz_type', 'male']]
print(DF_SUPERDATASET.columns)
ATRIBUTOS = DF_SUPERDATASET.columns[1:]

print('QUANTIDADE DE ATRIBUTOS: ', ATRIBUTOS)
...
def load_embeddings():
    ...
    cc_embeddings = np.vstack(DF_SUPERDATASET["face embedding"].values)
    cc_labels = DF_SUPERDATASET[["fitz_type", "male"]].values
...
def manipulating_embeddings(cc_embeddings, cc_labels):
    ...
    for i in range(y_train.shape[1]):
        ...
        y_train_input.append(to_categorical(train_col, num_classes)) if num_classes > 2 else y_train_input.append(train_col)
        y_test_input.append(to_categorical(test_col, num_classes)) if num_classes > 2 else y_test_input.append(test_col)
        y_valid_input.append(to_categorical(valid_col, num_classes)) if num_classes > 2 else y_valid_input.append(valid_col)

    global LABEL_ENCODERS
    LABEL_ENCODERS = label_encoders
    with open('superdataset_label_encoders.pkl', 'wb') as f:
...
def training_model(model, trainX, testX, trainy_input, testy_input):
    ...
    model_name = f'modelo_superdataset_{str(EPOCHS)}_{str(BATCH_SIZE)}_{str(INITIAL_LR)}.keras'
...
if __name__ == "__main__":
    ...
    tf.keras.utils.plot_model(model, to_file=f'modelo_superdataset_{str(EPOCHS)}_{str(BATCH_SIZE)}_{str(INITIAL_LR)}.png', show_shapes=True, show_layer_names=True)

    compiling_model(model)

    history = training_model(model, X_train, X_test, y_train_input, y_test_input)
    history_df = pd.DataFrame(history.history)
    history_df.to_csv(f'modelo_superdataset_{str(EPOCHS)}_{(BATCH_SIZE)}.csv')