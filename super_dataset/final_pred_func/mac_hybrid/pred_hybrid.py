...
if __name__ == "__main__":
    ...
    models = [[MODEL_HYBRID, "mac_superdataset"]]

    df_pred, df_train = compare_pred_func(models, image_path)

    print(df_pred)#, df_train)
    
    df_pred.to_csv(f"mac_superdataset_{image_path.split('/')[-1]}.csv")
    df_train.to_csv(f"mac_superdataset_{image_path.split('/')[-1]}.csv")