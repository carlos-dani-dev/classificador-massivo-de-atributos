...
DF_SUPERDATASET = pd.read_pickle('../../cross_labelling/casual_annotated.pkl')
...
def training_model(model, trainX, testX, trainy_input, testy_input):
    ...
    model_name = f'modelo_hybrid_{str(EPOCHS)}_{str(BATCH_SIZE)}_{str(INITIAL_LR)}.keras'
...
if __name__ == "__main__":
    ...
    tf.keras.utils.plot_model(model, to_file=f'modelo_hybrid_{str(EPOCHS)}_{str(BATCH_SIZE)}_{str(INITIAL_LR)}.png', show_shapes=True, show_layer_names=True)

    compiling_model(model)

    history = training_model(model, X_train, X_test, y_train_input, y_test_input)
    history_df = pd.DataFrame(history.history)
    history_df.to_csv(f'modelo_hybrid_{str(EPOCHS)}_{(BATCH_SIZE)}.csv')